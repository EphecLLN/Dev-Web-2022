...
router.get("/user/:id", (req, res,) => {
  db.connection.query(`SELECT id, pseudo FROM users WHERE id=
  ${db.connection.escape(req.params.id)};`, (err, rows) => {
    if(err) throw err
    if(rows.length == 1) {
...
router.post("/register", jsonParser, (req, res,) => {
  db.connection.query(`SELECT pseudo FROM users WHERE pseudo=
  ${db.connection.escape(req.body.pseudo)};`, (err, rows) => {
    if(err) throw err
    if(rows.length == 0) {
...
router.delete("/user/:id", jsonParser, (req, res,) => {
  db.connection.query(`DELETE FROM users WHERE id=
  ${req.params.id};`, (err, _rows) => {
    if(err) throw err
...
router.post("/login", jsonParser, (req, res,) => {
  ...
  ${db.connection.escape(req.body.pseudo)};`, (err, rows) => {
    if(err) throw err
    if(rows.length == 0) {
      db.connection.query(`SELECT SALT, PASSWORD FROM users WHERE pseudo=
      '${req.body.pseudo}';`, (err, rows) => {
        if(err) throw err
